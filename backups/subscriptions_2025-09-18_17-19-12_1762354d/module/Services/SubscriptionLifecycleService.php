 ...
 *
 * Responsável pelo gerenciamento do ciclo de vida de assinaturas:
 * - Pausar e retomar assinaturas
 * - Cancelar assinaturas (imediato ou no final do período)
 * - Upgrade e downgrade de planos
 * - Reativação de assinaturas canceladas
 * - Extensão de trials
 * - Histórico de eventos do lifecycle
 * - Agendamento de ações futuras
 *
 * Segue os princípios SOLID:
 * - S: Single Responsibility - Gerencia apenas lifecycle de assinatura
 * - O: Open/Closed - Extensível via herança
 * - L: Liskov Substitution - Implementa ServiceInterface
 * - I: Interface Segregation - Métodos específicos de lifecycle
 * - D: Dependency Inversion - Depende de abstrações
 */
class SubscriptionLifecycleService implements ServiceInterface
{
    ...
    public function isHealthy(): bool
    {
        try {
            ...
            $this->logger->error('SubscriptionLifecycleService health check failed', [
                'error' => $e->getMessage()
            ]);
            return false;
        }
    ...
    public function getConfig(): array
    {
        return [
            ...
            'supported_actions' => [
                ...
                'reactivate', 'extend_trial', 'schedule'
            ],
            'cancel_policies' => ['immediate', 'period_end'],
            'upgrade_billing' => 'prorated',
            'downgrade_billing' => 'period_end',
