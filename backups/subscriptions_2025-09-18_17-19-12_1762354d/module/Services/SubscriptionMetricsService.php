 ...
 *
 * Responsável pelas operações de analytics e métricas:
 * - Métricas gerais de assinatura (MRR, ARR, churn)
 * - Análise de churn e retenção
 * - Métricas de receita e crescimento
 * - Análise de saúde das assinaturas
 * - Análise de coorte
 * - Performance por plano
 * - Previsões e forecasting
 *
 * Segue os princípios SOLID:
 * - S: Single Responsibility - Gerencia apenas métricas de assinatura
 * - O: Open/Closed - Extensível via herança
 * - L: Liskov Substitution - Implementa ServiceInterface
 * - I: Interface Segregation - Métodos específicos de métricas
 * - D: Dependency Inversion - Depende de abstrações
 */
class SubscriptionMetricsService implements ServiceInterface
{
    ...
    public function isHealthy(): bool
    {
        try {
            ...
            $this->logger->error('SubscriptionMetricsService health check failed', [
                'error' => $e->getMessage()
            ]);
            return false;
        }
    ...
    /**
     * Obtém métricas de assinatura (método de negócio)
     */
    ...
    /**
     * Obtém métricas do serviço (ServiceInterface)
     */
    ...
    public function getConfig(): array
    {
        return [
            ...
            'version' => $this->getVersion(),
            'metrics_types' => [
                'mrr', 'arr', 'churn_rate', 'ltv', 'arpu',
                'nrr', 'grr', 'trial_conversion'
            ],
            'analysis_types' => [
                'churn', 'cohort', 'revenue', 'forecast',
                'plan_performance', 'health'
            ],
            'forecast_scenarios' => ['optimistic', 'realistic', 'pessimistic'],
            'cohort_types' => ['monthly', 'weekly', 'quarterly'],
