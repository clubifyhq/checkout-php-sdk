<?php

/**
 * Template para Repository Interface - Clubify Checkout SDK
 *
 * Este template define o contrato que todos os repositories de {Entity} devem seguir.
 * Estende RepositoryInterface para ter os métodos CRUD básicos.
 *
 * INSTRUÇÕES DE USO:
 * 1. Substitua {Entity} pelo nome da entidade (ex: Order)
 * 2. Substitua {entity} pela versão lowercase (ex: order)
 * 3. Adicione métodos específicos do domínio
 * 4. Documente todos os métodos com @param, @return e @throws
 *
 * EXEMPLO:
 * - {Entity} = Order
 * - {entity} = order
 */

namespace Clubify\Checkout\Modules\{ModuleName}\Contracts;

use Clubify\Checkout\Contracts\RepositoryInterface;

/**
 * {Entity} Repository Interface
 *
 * Define o contrato para repositories que gerenciam {Entity}s.
 * Estende RepositoryInterface para incluir métodos CRUD básicos:
 * - create(), findById(), update(), delete()
 * - findAll(), count(), exists()
 *
 * Adiciona métodos específicos para operações de {Entity}:
 *
 * @package Clubify\Checkout\Modules\{ModuleName}\Contracts
 * @version 2.0.0
 * @author Clubify Checkout Team
 */
interface {Entity}RepositoryInterface extends RepositoryInterface
{
    // ==============================================
    // DOMAIN-SPECIFIC METHODS
    // Customize these methods for your entity
    // ==============================================

    /**
     * Find {entity} by specific field (common pattern)
     *
     * @param string $fieldValue The value to search for
     * @return array|null {Entity} data or null if not found
     * @throws \Exception When search fails
     */
    public function findBy{Field}(string $fieldValue): ?array;

    /**
     * Find {entity}s by tenant (multi-tenant pattern)
     *
     * @param string $tenantId Tenant ID
     * @param array $filters Additional filters
     * @return array Array of {entity}s
     * @throws \Exception When search fails
     */
    public function findByTenant(string $tenantId, array $filters = []): array;

    /**
     * Update {entity} status (common pattern)
     *
     * @param string ${entity}Id {Entity} ID
     * @param string $status New status
     * @return bool True if updated successfully
     * @throws \Exception When update fails
     */
    public function updateStatus(string ${entity}Id, string $status): bool;

    /**
     * Get {entity} statistics (analytics pattern)
     *
     * @param array $filters Optional filters
     * @return array Statistics data
     * @throws \Exception When calculation fails
     */
    public function getStats(array $filters = []): array;

    /**
     * Bulk create {entity}s (performance pattern)
     *
     * @param array ${entity}sData Array of {entity} data
     * @return array Result with created {entity}s
     * @throws \Exception When bulk creation fails
     */
    public function bulkCreate(array ${entity}sData): array;

    /**
     * Bulk update {entity}s (performance pattern)
     *
     * @param array $updates Array of updates with {entity}Id => data
     * @return array Result with updated {entity}s
     * @throws \Exception When bulk update fails
     */
    public function bulkUpdate(array $updates): array;

    /**
     * Search {entity}s with advanced criteria (search pattern)
     *
     * @param array $criteria Search criteria
     * @param array $options Search options (sort, limit, offset)
     * @return array Search results with pagination
     * @throws \Exception When search fails
     */
    public function search(array $criteria, array $options = []): array;

    /**
     * Archive {entity} (soft delete pattern)
     *
     * @param string ${entity}Id {Entity} ID
     * @return bool True if archived successfully
     * @throws \Exception When archiving fails
     */
    public function archive(string ${entity}Id): bool;

    /**
     * Restore archived {entity} (soft delete pattern)
     *
     * @param string ${entity}Id {Entity} ID
     * @return bool True if restored successfully
     * @throws \Exception When restore fails
     */
    public function restore(string ${entity}Id): bool;

    /**
     * Get {entity} history/audit trail (audit pattern)
     *
     * @param string ${entity}Id {Entity} ID
     * @param array $options History options
     * @return array History records
     * @throws \Exception When history retrieval fails
     */
    public function getHistory(string ${entity}Id, array $options = []): array;

    // ==============================================
    // RELATIONSHIP METHODS
    // Add methods for managing relationships
    // ==============================================

    /**
     * Get related entities (relationship pattern)
     *
     * @param string ${entity}Id {Entity} ID
     * @param string $relationType Type of relationship
     * @param array $options Relationship options
     * @return array Related entities
     * @throws \Exception When relationship retrieval fails
     */
    public function getRelated(string ${entity}Id, string $relationType, array $options = []): array;

    /**
     * Add relationship (relationship pattern)
     *
     * @param string ${entity}Id {Entity} ID
     * @param string $relatedId Related entity ID
     * @param string $relationType Type of relationship
     * @param array $metadata Optional relationship metadata
     * @return bool True if relationship created successfully
     * @throws \Exception When relationship creation fails
     */
    public function addRelationship(string ${entity}Id, string $relatedId, string $relationType, array $metadata = []): bool;

    /**
     * Remove relationship (relationship pattern)
     *
     * @param string ${entity}Id {Entity} ID
     * @param string $relatedId Related entity ID
     * @param string $relationType Type of relationship
     * @return bool True if relationship removed successfully
     * @throws \Exception When relationship removal fails
     */
    public function removeRelationship(string ${entity}Id, string $relatedId, string $relationType): bool;

    // ==============================================
    // CACHING METHODS
    // Methods for cache management
    // ==============================================

    /**
     * Invalidate {entity} cache
     *
     * @param string ${entity}Id {Entity} ID
     * @return void
     */
    public function invalidateCache(string ${entity}Id): void;

    /**
     * Warm up cache for {entity}
     *
     * @param string ${entity}Id {Entity} ID
     * @return void
     */
    public function warmCache(string ${entity}Id): void;

    /**
     * Clear all {entity} caches
     *
     * @return void
     */
    public function clearAllCache(): void;
}