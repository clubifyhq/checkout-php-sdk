 ...
 */
class OrderStatusService extends BaseService implements ServiceInterface
{
    /**
    ...
    {
        try {
            // Test basic functionality with a status statistics call
            $this->getStatusStatistics(['limit' => 1]);
            return true;
        } catch (\Exception $e) {
            $this->logger->error('OrderStatusService health check failed', [
                'error' => $e->getMessage()
            ]);
            return false;
        }
    ...
    public function getMetrics(): array
    {
        return [
            ...
            'allowed_statuses' => array_keys(self::ALLOWED_TRANSITIONS),
            'notify_statuses' => self::NOTIFY_CUSTOMER_STATUSES,
            'timestamp' => time()
        ];
    }
    ...
    public function getConfig(): array
    {
        return [
            ...
            'notify_customer_statuses' => self::NOTIFY_CUSTOMER_STATUSES,
            'endpoints' => [
                'update_status' => '/orders/{id}/status',
                'status_history' => '/orders/{id}/status-history',
                'bulk_update' => '/orders/bulk-status-update',
                'statistics' => '/orders/status-statistics'
            ],
            'features' => [
