<?php

/**
 * Template para Repository Interface - Clubify Checkout SDK
 *
 * Este template define o contrato que todos os repositories de Webhook devem seguir.
 * Estende RepositoryInterface para ter os métodos CRUD básicos.
 *
 * INSTRUÇÕES DE USO:
 * 1. Substitua Webhook pelo nome da entidade (ex: Order)
 * 2. Substitua webhook pela versão lowercase (ex: order)
 * 3. Adicione métodos específicos do domínio
 * 4. Documente todos os métodos com @param, @return e @throws
 *
 * EXEMPLO:
 * - Webhook = Order
 * - webhook = order
 */

namespace Clubify\Checkout\Modules\Webhooks\Contracts;

use Clubify\Checkout\Contracts\RepositoryInterface;

/**
 * Webhook Repository Interface
 *
 * Define o contrato para repositories que gerenciam Webhooks.
 * Estende RepositoryInterface para incluir métodos CRUD básicos:
 * - create(), findById(), update(), delete()
 * - findAll(), count(), exists()
 *
 * Adiciona métodos específicos para operações de Webhook:
 *
 * @package Clubify\Checkout\Modules\Webhooks\Contracts
 * @version 2.0.0
 * @author Clubify Checkout Team
 */
interface WebhookRepositoryInterface extends RepositoryInterface
{
    // ==============================================
    // DOMAIN-SPECIFIC METHODS
    // Customize these methods for your entity
    // ==============================================

    /**
     * Find webhook by specific field (common pattern)
     *
     * @param string $fieldValue The value to search for
     * @return array|null Webhook data or null if not found
     * @throws \Exception When search fails
     */
    public function findByEmail(string $fieldValue): ?array;

    /**
     * Find webhooks by tenant (multi-tenant pattern)
     *
     * @param string $tenantId Tenant ID
     * @param array $filters Additional filters
     * @return array Array of webhooks
     * @throws \Exception When search fails
     */
    public function findByTenant(string $tenantId, array $filters = []): array;

    /**
     * Update webhook status (common pattern)
     *
     * @param string $webhookId Webhook ID
     * @param string $status New status
     * @return bool True if updated successfully
     * @throws \Exception When update fails
     */
    public function updateStatus(string $webhookId, string $status): bool;

    /**
     * Get webhook statistics (analytics pattern)
     *
     * @param array $filters Optional filters
     * @return array Statistics data
     * @throws \Exception When calculation fails
     */
    public function getStats(array $filters = []): array;

    /**
     * Bulk create webhooks (performance pattern)
     *
     * @param array $webhooksData Array of webhook data
     * @return array Result with created webhooks
     * @throws \Exception When bulk creation fails
     */
    public function bulkCreate(array $webhooksData): array;

    /**
     * Bulk update webhooks (performance pattern)
     *
     * @param array $updates Array of updates with webhookId => data
     * @return array Result with updated webhooks
     * @throws \Exception When bulk update fails
     */
    public function bulkUpdate(array $updates): array;

    /**
     * Search webhooks with advanced criteria (search pattern)
     *
     * @param array $filters Search filters
     * @param array $sort Sorting criteria
     * @param int $limit Maximum number of results
     * @param int $offset Number of results to skip
     * @return array Search results with pagination
     * @throws \Exception When search fails
     */
    public function search(array $filters, array $sort = [], int $limit = 100, int $offset = 0): array;

    /**
     * Archive webhook (soft delete pattern)
     *
     * @param string $webhookId Webhook ID
     * @return bool True if archived successfully
     * @throws \Exception When archiving fails
     */
    public function archive(string $webhookId): bool;

    /**
     * Restore archived webhook (soft delete pattern)
     *
     * @param string $webhookId Webhook ID
     * @return bool True if restored successfully
     * @throws \Exception When restore fails
     */
    public function restore(string $webhookId): bool;

    /**
     * Get webhook history/audit trail (audit pattern)
     *
     * @param string $webhookId Webhook ID
     * @param array $options History options
     * @return array History records
     * @throws \Exception When history retrieval fails
     */
    public function getHistory(string $webhookId, array $options = []): array;

    // ==============================================
    // RELATIONSHIP METHODS
    // Add methods for managing relationships
    // ==============================================

    /**
     * Get related entities (relationship pattern)
     *
     * @param string $webhookId Webhook ID
     * @param string $relationType Type of relationship
     * @param array $options Relationship options
     * @return array Related entities
     * @throws \Exception When relationship retrieval fails
     */
    public function getRelated(string $webhookId, string $relationType, array $options = []): array;

    /**
     * Add relationship (relationship pattern)
     *
     * @param string $webhookId Webhook ID
     * @param string $relatedId Related entity ID
     * @param string $relationType Type of relationship
     * @param array $metadata Optional relationship metadata
     * @return bool True if relationship created successfully
     * @throws \Exception When relationship creation fails
     */
    public function addRelationship(string $webhookId, string $relatedId, string $relationType, array $metadata = []): bool;

    /**
     * Remove relationship (relationship pattern)
     *
     * @param string $webhookId Webhook ID
     * @param string $relatedId Related entity ID
     * @param string $relationType Type of relationship
     * @return bool True if relationship removed successfully
     * @throws \Exception When relationship removal fails
     */
    public function removeRelationship(string $webhookId, string $relatedId, string $relationType): bool;

    // ==============================================
    // CACHING METHODS
    // Methods for cache management
    // ==============================================

    /**
     * Invalidate webhook cache
     *
     * @param string $webhookId Webhook ID
     * @return void
     */
    public function invalidateCache(string $webhookId): void;

    /**
     * Warm up cache for webhook
     *
     * @param string $webhookId Webhook ID
     * @return void
     */
    public function warmCache(string $webhookId): void;

    /**
     * Clear all webhook caches
     *
     * @return void
     */
    public function clearAllCache(): void;
}
